...
var shiftLetters = function (string) {
  return string
    .split("")
    .map((c, i) => String.fromCharCode(string.charCodeAt(i) + 1))
    .join("");
};
...
var countLetters = function (string) {
  return string.split("").reduce(
    (previusVal, currentVal) => ({
      ...previusVal,
      [currentVal]: previusVal[currentVal] + 1 || 1,
    }),
    {}
  );
};
...
var isPresent = function (string, target) {
  return string
    .split("")
    .reduce((prev, current) => (current === target || prev ? (prev = true) : false), 0);
};
...
var decode = function (string) {
  return string
    .split(" ")
    .map(
      (letter) => letter.split("").reduce((prev, current) => parseInt(prev) + parseInt(current)),
      0
    )
